...
import type { FacultyMoreStackParamList } from '../../types/navigation'
import type { LucideIcon } from 'lucide-react-native'

type Nav = NativeStackNavigationProp<FacultyMoreStackParamList>

const menuItems: { key: keyof FacultyMoreStackParamList; label: string; Icon: LucideIcon; desc: string }[] = [
  { key: 'FacultyLeaves', label: 'Leave Requests', Icon: CalendarDays, desc: 'Approve or reject leaves' },
  { key: 'ChatList', label: 'Parent Chat', Icon: MessageCircle, desc: 'Chat with parents' },
  { key: 'Notices', label: 'Notices', Icon: FileText, desc: 'College notices' },
  { key: 'Calendar', label: 'Calendar', Icon: Calendar, desc: 'Academic calendar' },
  { key: 'Profile', label: 'Profile', Icon: User, desc: 'Your account' },
]

export function FacultyMoreMenuScreen() {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scroll}>
      {menuItems.map((item, i) => (
        <Animated.View key={item.key} entering={FadeInDown.duration(400).delay(i * 60).springify()}>
          <TouchableOpacity style={styles.card} onPress={() => nav.navigate(item.key as any)} activeOpacity={0.7}>
